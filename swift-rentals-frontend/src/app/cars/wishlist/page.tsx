...
export default function CarWishList() {
    ...
    const getWishList = async () => {
        try {
            const response = await carAPI.getCarsInWishList();

            if (response?.status === 200) {
                const cars: Car[] = response?.data?.carsInWishlist.map(
                    ({ _id, make, model, manufacturing_year, is_available, price, images, description, location, Features, ratings }: Car) => ({
                        _id,
                        make,
                        model,
                        manufacturing_year,
                    ...
                    allCars.map((item: Car, index) => (
                        <div key={index} className="flex flex-col flex-wrap border bg-white rounded-2xl overflow-hidden md:flex-row">
                            <div onClick={() => handleCarClick(item)} className="w-full md:w-[300px]">
                                <Image
                                    src={item?.images && item?.images[0] ? item?.images[0] : '/images/car-placeholder.png'}
                                    alt="Car"
                                    width={300}
                                    height={180}
                                    layout="responsive"
