...
const CarSchema = z.object({
  ...
  manufacturing_year: z
    .string()
    .min(4, { message: "manufacturing_year must be valid" })
    .max(4),
  is_available: z.boolean({ required_error: "is_available is required" }),
  price: z
    .string()
    .min(1, { message: "Price is required" })
    .max(3, { message: "Price cannot be more than $999" }),
...
export default function AddCar() {
  ...
  const onSubmit: SubmitHandler<CarSchemaType> = async (data) => {
    try {
      ...
      const response = await carAPI.addCar(formData, token);
  ...
  const handleFeatureIncluded = (value: string) => {
    ...
    if (isFeatureIncluded) {
      setFeaturesIncluded(
        featuresIncluded.filter((feature) => feature !== value)
      );
    } else {
  ...
  return (
    <main className="min-min-h-screen w-full">
      <div className="container mx-auto md:max-w-[1050px] py-5">
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="form grid grid-cols-1 md:grid-cols-2 gap-10"
        >
          <div>
            <input
              type="file"
            ...
            <div className="flex flex-wrap gap-3 my-3">
              {feature?.featureList?.map((feature, index) => (
                <div
                  ...
                  className={`${
                    featuresIncluded.includes(feature._id)
                      ? "bg-blue-100"
                      : "bg-white"
